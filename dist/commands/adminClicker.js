...
function parseAdminClickerSettleCommand(text) {
...
function parseAdminCreditEnergyCommand(text) {
    const firstLine = text.split(/\r?\n/, 1)[0]?.trim() ?? "";
...
function parseAdminResetClickerCommand(text) {
    const firstLine = text.split(/\r?\n/, 1)[0]?.trim() ?? "";
    const target = firstLine.replace(/^\/admin_reset_clicker(?:@\S+)?\s*/i, "").trim();
    return target || null;
}
export function registerAdminClickerCommands(bot) {
