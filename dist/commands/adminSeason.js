...
function parseAdminPointsCommand(text, commandName) {
...
function parseAdminEventCommand(text) {
    const firstLine = text.split(/\r?\n/, 1)[0]?.trim() ?? "";
...
function parseSingleArgumentCommand(text, commandName) {
    const firstLine = text.split(/\r?\n/, 1)[0]?.trim() ?? "";
    const value = firstLine.replace(new RegExp(`^/${commandName}(?:@\\S+)?\\s*`, "i"), "").trim();
    return value || null;
}
function describeDynamicEventInput(eventType) {
...
export function registerAdminSeasonCommands(bot) {
    ...
    bot.command("admin_status", async (ctx) => {
        ...
        try {
            ...
            await ctx.reply([
                ...
                `Top 5 Users: ${status.topUsers.length > 0
                    ? status.topUsers
                        .map((entry) => {
    ...
    bot.command("admin_event", async (ctx) => {
        ...
        try {
            ...
            else if (SUPPORTED_EVENT_TYPES.includes(eventType)) {
                const txHash = parsed.maybeValue && !/^-?\d+(\.\d+)?$/.test(parsed.maybeValue) ? parsed.maybeValue : parsed.maybeTxHash;
    ...
    bot.command("admin_scanner_status", async (ctx) => {
        ...
        await ctx.reply([
            ...
            `Recent Errors: ${status.recentErrors.length > 0
        ...
        try {
            ...
            await ctx.reply([
                ...
                `Diagnostics: ${result.diagnostics.length > 0
                    ? result.diagnostics
                        .slice(0, 5)
                        .map((entry) => `${entry.txHash.slice(0, 12)}... [${entry.instructionNames.join(",") || "-"}]${entry.reason ? ` ${entry.reason}` : ""}`)
                        .join(" | ")
                    : "none"}`
            ].join("\n"));
        }
