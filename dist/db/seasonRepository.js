...
function slugifySeasonName(name) {
    return name
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
...
export async function upsertSeason(data) {
...
export async function startSeasonNow(params) {
    const activeSeason = await getActiveSeason();
    if (activeSeason) {
...
export async function recalculateUserSeasonStats(userId, seasonId) {
    return prisma.$transaction((tx) => recalculateUserSeasonStatsTx(tx, userId, seasonId));
}
export async function recalculateUserSeasonStatsTx(tx, userId, seasonId) {
    ...
    await Promise.all(leaderboard.map((entry, index) => tx.userSeasonStats.update({
        where: { id: entry.id },
        data: {
            rank: index + 1
        }
