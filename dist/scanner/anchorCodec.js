...
export function encodeAnchorDiscriminator(namespace, name) {
...
export function mindAtomicToDecimal(value) {
    return Number(value) / 1_000_000_000;
}
export function rigTypeFromIndex(value) {
    if (value === 0) {
        return "starter";
    }
    if (value === 1) {
...
export function decodeInstructionName(dataBase58, instructionNames) {
    ...
    for (const name of instructionNames) {
        if (signature === encodeAnchorDiscriminator("global", name) ||
...
export function decodeBuyContractArgs(dataBase58) {
    const data = decodeBase58(dataBase58);
...
export function decodeStakeMindArgs(dataBase58) {
    const data = decodeBase58(dataBase58);
    const cursor = { offset: 8 };
    return {
...
export function decodeAnchorEvent(buffer) {
    if (buffer.length < 8) {
        return null;
    }
...
export function decodeUserMiningProfileAccount(data) {
    const cursor = { offset: 8 };
    return {
...
export function decodeMinerPositionAccount(data) {
    const cursor = { offset: 8 };
...
export function decodeMinerPositionAccountSafe(data) {
    const cursor = { offset: 8 };
...
export function decodeUserStakeAccount(data) {
    const cursor = { offset: 8 };
    return {
        owner: readPublicKey(data, cursor),
...
export function isExpectedAccountDiscriminator(data, accountName) {
    if (data.length < 8) {
        return false;
    }
