...
import { decodeAnchorEvent, decodeBuyContractArgs, decodeInstructionName, decodeMinerPositionAccountSafe, decodeStakeMindArgs, decodeUserMiningProfileAccount, decodeUserStakeAccount, isExpectedAccountDiscriminator, mindAtomicToDecimal, rigTypeFromIndex } from "./anchorCodec.js";
const DEFAULT_RPC_URL = "https://rpc.mainnet.x1.xyz";
const DEFAULT_PROGRAM_ID = "uaDkkJGLLEY3kFMhhvrh5MZJ6fmwCmhNf8L7BZQJ9Aw";
const SUPPORTED_INSTRUCTIONS = ["buyContract", "renewRig", "renewRigWithBuff", "claimMind", "stakeMind"];
const PARSED_TRANSACTION_BATCH_SIZE = 10;
function positionIndexSeed(index) {
    const seed = Buffer.alloc(8);
    seed.writeBigUInt64LE(BigInt(index));
    return seed;
}
function findIdlPath() {
...
function loadProgramArtifact() {
    ...
    try {
        const parsed = JSON.parse(readFileSync(idlPath, "utf8"));
        return {
            idlPath,
            instructionNames: parsed.instructions?.map((entry) => entry.name).filter((entry) => Boolean(entry)) ?? [],
...
function rigTypeFromPurchaseEvent(hp, durationDays) {
...
function formatRawSummary(txHash, instructionNames, eventNames) {
...
export class RealX1FactoryAdapter {
    connection = new Connection(env.x1RpcUrl ?? DEFAULT_RPC_URL, "confirmed");
    programId = new PublicKey(env.x1FactoryProgramId ?? DEFAULT_PROGRAM_ID);
    artifact = loadProgramArtifact();
    diagnostics = new Map();
    ...
    getWalletDiagnostics(wallet) {
        return (this.diagnostics.get(wallet) ?? {
            wallet,
    ...
    async inspectWallet(wallet, sinceSlot) {
    ...
    async getUserFactoryState(wallet) {
        ...
        const positionKeys = Array.from({ length: totalPositions }, (_, index) => PublicKey.findProgramAddressSync([Buffer.from("position"), owner.toBuffer(), positionIndexSeed(index)], this.programId)[0]);
    ...
    async captureDiagnostics(wallet) {
    ...
    async loadWalletEvents(wallet, sinceSlot) {
        ...
        const parsedTransactions = await getParsedTransactionsInBatches(this.connection, filtered.map((entry) => entry.signature));
        const candidates = [];
        const events = [];
        ...
        for (let index = 0; index < filtered.length; index += 1) {
            ...
            const walletPurchaseEvents = eventMatches.filter((entry) => entry.name === "ContractPurchased" && entry.fields.owner === wallet);
            const walletClaimEvents = eventMatches.filter((entry) => entry.name === "MindClaimed" && entry.fields.owner === wallet);
            const walletStakeEvents = eventMatches.filter((entry) => entry.name === "MindStaked" && entry.fields.owner === wallet);
        ...
        if (pendingRenewals.length > 0) {
            const renewalAccounts = await this.connection.getMultipleAccountsInfo(pendingRenewals.map((entry) => new PublicKey(entry.positionAddress)), "confirmed");
    ...
    extractProgramInstructions(parsed) {
    ...
    extractProgramEvents(logMessages) {
    ...
    getInstructionAccountRoles(name) {
        if (name === "buyContract") {
    ...
    purchaseEventTypeFromRigType(rigType) {
        if (rigType === "starter") {
    ...
    renewalEventTypeFromRigType(rigType) {
        if (rigType === "starter") {
