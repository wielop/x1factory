import { POINT_VALUES } from "../config/points.js";
...
export async function runScannerCycle(deps) {
    ...
    for (const registration of registrations) {
        try {
            ...
            const derivedEvents = recentEvents.length > 0
                ? recentEvents
                : deriveEventsFromSnapshotDiff(registration.wallet.address, previousState, currentState);
            for (const event of derivedEvents) {
                const basePoints = POINT_VALUES[event.category] ?? 0;
                const amount = basePoints * (event.multiplier ?? 1);
                if (amount <= 0) {
                    continue;
                }
                const result = await addSeasonPoints(registration.userId, season.id, amount, event.category, event.reason, event.txHash);
                if (result.created) {
                    logger.info({
                        seasonId: season.id,
