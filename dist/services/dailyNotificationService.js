...
async function getAllActiveUsers(seasonId) {
    return prisma.userSeasonStats.findMany({
        where: { seasonId },
        include: { user: true },
    });
}
async function checkinDoneToday(userId, seasonId) {
...
async function claimDoneToday(userId, seasonId) {
    const dayStart = startOfUtcDay();
    const dayEnd = new Date(dayStart.getTime() + 86_400_000);
    const row = await prisma.seasonPoint.findFirst({
...
async function todayPoints(userId, seasonId) {
    const dayStart = startOfUtcDay();
    const dayEnd = new Date(dayStart.getTime() + 86_400_000);
...
export async function sendNoonReminders() {
    const season = await getActiveSeason();
    if (!season)
        return;
    const users = await getAllActiveUsers(season.id);
    ...
    for (const stats of users) {
        ...
        const missing = [];
        if (!checkin)
            missing.push("⚡ /checkin — Daily Check-in (+10 pts)");
        if (!claim)
            missing.push("💎 Claim MIND on X1Factory (+5–150 pts)");
        const lines = [
...
export async function sendEveningRecaps() {
    const season = await getActiveSeason();
    if (!season)
        return;
    const users = await getAllActiveUsers(season.id);
    ...
    for (const stats of users) {
        ...
        if (checkin && claim) {
            ...
            const stillMissing = [];
            if (!checkin)
                stillMissing.push("check-in (/checkin)");
            if (!claim)
                stillMissing.push("claim MIND on X1Factory");
