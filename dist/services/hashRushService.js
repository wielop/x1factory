...
function getNextUpgrade(type, currentLevel) {
    if (type === "pickaxe") {
...
function activeBoostsWhere(now) {
    return {
        expiresAt: {
            gt: now
        }
...
async function upsertHashRushUser(from) {
...
async function ensureProfile(userId, seasonId, now) {
...
async function ensureEnergyBalance(userId, seasonId) {
...
async function refreshEnergy(profile, now) {
...
async function resetDailyClicksIfNeeded(profile, now) {
...
async function loadDashboard(user, season, now = new Date()) {
...
async function getUserDashboardFromTelegram(from) {
...
export async function getHashRushDashboard(from) {
    return getUserDashboardFromTelegram(from);
}
export async function mineHashRush(from) {
...
export async function collectHashRush(from) {
    const dashboard = await getUserDashboardFromTelegram(from);
    const now = new Date();
...
export async function buyHashRushBoost(from, boostType) {
...
export async function resetHashRushForUser(userId) {
    const season = await getActiveSeason();
    if (!season) {
        throw new Error("No active season found.");
    }
...
export function renderHashRushDashboard(dashboard) {
...
export function renderHashRushUpgrades(dashboard) {
...
export function renderHashRushBoosts(dashboard) {
    return [
        "Hash Rush Boosts",
        "",
