import { CLAIM_MIND_DAILY_CAP, CLAIM_MIND_DAILY_THRESHOLDS, FIXED_EVENT_TYPES, POINT_VALUES, STAKE_THRESHOLDS, STREAK_BONUSES, SWAP_DAILY_CAP, SWAP_THRESHOLDS } from "../config/points.js";
...
function formatMindAmount(amount) {
...
function resolvePointOccurredAt(metadata) {
    return (parseMetadataDate(metadata?.eventOccurredAt) ??
...
function getClaimPointsForAmount(amount) {
...
function getNextClaimThreshold(amount) {
...
function getStakeThresholdForAmount(amount) {
...
function getNextStakeThreshold(amount) {
    const ascending = [...STAKE_THRESHOLDS].sort((left, right) => left.minAmount - right.minAmount);
    return ascending.find((threshold) => amount < threshold.minAmount) ?? null;
}
export async function addPoints(userId, seasonId, amount, category, reason, metadata) {
...
export async function addSeasonPoints(userId, seasonId, amount, category, reason, txHash, metadata) {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        let detectedEventId = null;
        let walletId = fullUser.activeWalletId ?? null;
        if (txHash) {
...
export async function processEvent(userId, seasonId, eventType, metadata) {
    if (FIXED_EVENT_TYPES.includes(eventType)) {
        const amount = POINT_VALUES[eventType];
...
export async function processDailyClaim(userId, seasonId, claimedMindAmount, date, claimIncrementAmount) {
    ...
    if (user) {
        ...
        if (nextThreshold) {
            progressLines.push("", `Next MIND crate: ${formatMindAmount(nextThreshold.minAmount)} MIND`, `Missing: ${formatMindAmount(nextThreshold.minAmount - claimedMindAmount)} MIND`);
        }
        else {
...
export async function processStakeSnapshot(userId, seasonId, stakedMindAmount, actualStakedMindAmount = stakedMindAmount, stakeBaselineAmount = 0) {
    ...
    const alreadyAwarded = existingPoints._sum.points ?? 0;
    const pointsToAward = Math.max(0, targetPoints - alreadyAwarded);
    const result = await addPoints(userId, seasonId, pointsToAward, "stake_snapshot", reachedThreshold
    ...
    if (user) {
        ...
        if (nextThreshold) {
            progressLines.push("", `Next stake crate: ${formatMindAmount(nextThreshold.minAmount)} MIND`, `Missing: ${formatMindAmount(nextThreshold.minAmount - stakedMindAmount)} MIND`);
        }
        else {
...
export async function processSwap(userId, seasonId, usdCents, date) {
    ...
    return addPoints(userId, seasonId, pointsToAward, "swap_mind_xnt", `Swap $${(usdCents / 100).toFixed(2)}`, { usdCents, tierPoints, suppressDefaultNotification: true });
}
export async function processDailyCheckin(userId, seasonId, date) {
    const normalizedDate = normalizeDate(date);
    const dayStart = startOfUtcDay(normalizedDate);
    const dayEnd = addUtcDays(dayStart, 1);
    ...
    await addPoints(userId, seasonId, streakBonus.points, `streak_bonus_${streakBonus.days}_day`, `${streakBonus.days}-day streak bonus`, {
        checkinDate: dayStart.toISOString(),
        streakLength
    });
...
async function notifyOvertakenUser(seasonId, overtakenRank, scorerUserId) {
    const overtakenStats = await prisma.userSeasonStats.findFirst({
        where: { seasonId, rank: overtakenRank },
        include: { user: true }
    });
    if (!overtakenStats || overtakenStats.userId === scorerUserId)
        return;
    ...
    await notifyTelegramUser(overtakenStats.user.telegramId, [
...
export async function checkSeasonEndNotifications() {
    ...
    for (const stats of topStats) {
        ...
        await notifyTelegramUser(stats.user.telegramId, [
...
function getEstimatedPrize(rank, total) {
    if (rank === 1)
