...
import { endActiveSeason, getActiveOrUpcomingSeason, getActiveSeason, startSeasonNow } from "../db/seasonRepository.js";
const SEASON_DURATION_DAYS = 21;
const BREAK_DURATION_DAYS = 7;
const DAY_IN_MS = 24 * 60 * 60 * 1000;
export function getSeasonTestingNotice(seasonName) {
    if (!seasonName) {
        return null;
    }
...
export async function getSeasonOverview() {
    ...
    const dayNumber = season.status === "ACTIVE"
        ? Math.max(1, Math.floor((now.getTime() - season.startsAt.getTime()) / DAY_IN_MS) + 1)
        : 0;
    const timeLeft = season.status === "ACTIVE"
...
export async function adminStartSeason(name) {
