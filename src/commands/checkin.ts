...
function getPanelUrl(): string { return env.miniAppUrl ?? PANEL_URL; }

export async function showDailyCheckin(ctx: BotContext): Promise<void> {
  const from = ctx.from;
  if (!from) return;
  ...
  if (!wallet) {
    await ctx.reply(
      [factoryHeader("DAILY CHECK-IN"), "", "Connect your wallet first to earn Season Points."].join("\n"),
  ...
  if (!season) {
    await ctx.reply(
      [factoryHeader("DAILY CHECK-IN"), "", "No active season right now. Check back soon."].join("\n"),
      mainMenuKeyboard(getPanelUrl())
    );
    return;
  }
...
export function registerCheckinCommand(bot: BotInstance): void {
