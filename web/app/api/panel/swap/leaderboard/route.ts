...
export async function GET(req: Request) {
  try {
    ...
    if (mode === "season" && activeSeason) {
      rows = await prisma.$queryRaw<SwapRow[]>`
        SELECT
          ...
          BOOL_OR(CASE WHEN sp.category = 'giga_swap_win'
            AND (sp.metadata->>'payout')::bigint =
              (SELECT MAX((sp2.metadata->>'payout')::bigint) FROM "SeasonPoint" sp2
               WHERE sp2."userId" = u.id AND sp2."seasonId" = ${activeSeason.id}
               AND sp2.category = 'giga_swap_win')
            THEN (sp.metadata->>'paidMind')::boolean ELSE NULL END)                  AS "bestPayoutMind"
        FROM "User" u
    ...
    const entries = rows.map((r, i) => ({
      ...
      bestPayoutLamports: r.bestPayoutLamports.toString(),
      bestPayoutMind: r.bestPayoutMind,
