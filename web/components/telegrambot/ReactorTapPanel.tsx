...
function ReactorCore({ pulse }: { pulse: boolean }) {
  return (
    <svg viewBox="0 0 960 960" className="h-full w-full" aria-hidden="true">
      <defs>
        <radialGradient id="coreGlow" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(480 480) scale(240)">
          <stop offset="0" stopColor="#ffffff" />
          <stop offset="0.2" stopColor="#b3fbff" />
          <stop offset="0.5" stopColor="#22d3ee" />
          <stop offset="1" stopColor="#041018" />
        </radialGradient>
        <linearGradient id="steel" x1="180" y1="180" x2="780" y2="780" gradientUnits="userSpaceOnUse">
      ...
      <circle cx="480" cy="480" r="360" fill="url(#steel)" stroke="#7b8794" strokeOpacity="0.14" strokeWidth="8" />
      <circle cx="480" cy="480" r="292" fill="#0c1118" stroke="#374151" strokeWidth="10" />
      <circle cx="480" cy="480" r="228" fill="#081018" stroke="#29323a" strokeWidth="8" />
      <circle cx="480" cy="480" r="186" fill="none" stroke="#5ee7ff" strokeOpacity="0.55" strokeWidth="18" />
      <circle cx="480" cy="480" r="144" fill="none" stroke="#5ee7ff" strokeOpacity="0.8" strokeWidth="12" />
      <circle cx="480" cy="480" r="100" fill="none" stroke="#7dfcff" strokeOpacity="0.92" strokeWidth="10" />
      <g filter="url(#blur)">
        <circle cx="480" cy="480" r="120" fill="url(#coreGlow)" />
        <circle cx="480" cy="480" r="64" fill="#d8ffff" />
        <circle cx="480" cy="480" r="26" fill="#ffffff" />
      </g>
      <g opacity="0.92">
        <rect x="452" y="182" width="56" height="116" rx="16" fill="url(#steel)" stroke="#84d8e8" strokeOpacity="0.3" />
        <rect x="452" y="662" width="56" height="116" rx="16" fill="url(#steel)" stroke="#84d8e8" strokeOpacity="0.3" />
        <rect x="182" y="452" width="116" height="56" rx="16" fill="url(#steel)" stroke="#84d8e8" strokeOpacity="0.3" />
        <rect x="662" y="452" width="116" height="56" rx="16" fill="url(#steel)" stroke="#84d8e8" strokeOpacity="0.3" />
      </g>
      <g filter="url(#blur)">
...
export function ReactorTapPanel() {
  ...
  const [tapsLeft, setTapsLeft] = useState(DEFAULT_CONFIG.dailyTapCap);
  ...
  const tapProgress = Math.min(100, ((config.dailyTapCap - tapsLeft) / config.dailyTapCap) * 100);

  function pushFeed(title: string, detail: string) {
    setFeed((current) => [{ title, detail }, ...current].slice(0, 5));
  }

  function pushBurst(value: string) {
    const id = burstIdRef.current + 1;
    burstIdRef.current = id;
    setBurst({ id, value, top: 24 + Math.floor(Math.random() * 42), left: 24 + Math.floor(Math.random() * 48) });
  ...
  function tapFactory() {
    ...
    setClaimableMind((current) => Number((current + TAP_GAIN).toFixed(2)));
    setSeasonMind((current) => Number((current + TAP_GAIN).toFixed(2)));
    setClaimStatus("none");
    setLastAction("Reactor tapped.");
    pushFeed("Reactor hit", `+${TAP_GAIN.toFixed(2)} MIND claimable.`);
    pushBurst(`+${TAP_GAIN.toFixed(2)}`);
  }
  ...
  return (
    <main className="min-h-screen bg-[#03060a] px-4 py-5 text-zinc-100 sm:px-6 lg:px-8">
      <div className="mx-auto flex min-h-[calc(100vh-2.5rem)] w-full max-w-5xl flex-col gap-4">
        <header className="rounded-3xl border border-cyan-400/15 bg-[#081018]/90 p-4">
        ...
        <section className="grid gap-4 xl:grid-cols-[1.35fr_0.85fr]">
          <div className="space-y-4">
            <div className="rounded-3xl border border-cyan-400/15 bg-[#061018]/90 p-4">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-2xl border border-white/5 bg-white/5 p-4">
                  ...
                  <div className="mt-2 text-2xl font-semibold text-white">{Math.round(tapProgress)}%</div>
                </div>
              </div>
              ...
              <div className="mt-4">
                <div className="mb-2 flex items-center justify-between text-xs text-zinc-500">
                  ...
                  <span>{config.dailyTapCap - tapsLeft}/{config.dailyTapCap}</span>
                </div>
                <div className="h-2 rounded-full bg-white/5">
                  <div className="h-2 rounded-full bg-gradient-to-r from-cyan-400 via-teal-300 to-emerald-300" style={{ width: `${tapProgress}%` }} />
                </div>
              </div>
            </div>

            <div className="rounded-3xl border border-cyan-400/15 bg-[#081018]/90 p-4">
              <div className="grid gap-3 md:grid-cols-3">
                <Button size="lg" onClick={startClaim} className="w-full">
                  Claim MIND
                </Button>
                <Button size="lg" variant="secondary" onClick={cancelClaim} className="w-full">
                  Cancel Claim
                </Button>
                <Button size="lg" variant="ghost" onClick={() => webAppRef.current?.close?.()} className="w-full">
                  Close
                </Button>
              </div>
          ...
          <div className="space-y-4">
            <div className="rounded-3xl border border-cyan-400/15 bg-[#081018]/90 p-4">
              ...
              <div className="mt-4 space-y-3">
                {feed.map((line) => (
                  <div key={line.title + line.detail} className="rounded-2xl border border-white/5 bg-white/5 p-3 text-sm text-zinc-300">
                    <div className="font-semibold text-zinc-100">{line.title}</div>
                    <div className="mt-1 leading-6 text-zinc-300/80">{line.detail}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-cyan-400/15 bg-[#081018]/90 p-4">
              ...
              <div className="mt-4 space-y-2 text-sm text-zinc-300">
                ...
                <div className="rounded-2xl border border-white/5 bg-white/5 px-3 py-2 text-zinc-400">
                  The reactor is the tap target. Buttons below handle claims and exit.
                </div>
              </div>
            </div>
          </div>
